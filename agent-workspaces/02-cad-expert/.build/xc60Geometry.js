...
const NO_SHADOW = new Set(["glass", "lightFront", "lightRear", "plate", "underbody"]);
// ---------------------------------------------------------------------------
// Small math helpers
// ---------------------------------------------------------------------------
...
function skinXAt(raw, y, z) {
    let r = 0;
    while (r < RINGS - 2 && raw[r + 1][20].z < z)
        r++;
    const z0 = raw[r][20].z;
...
function buildHullGeometry(grid) {
    const pos = [];
    const idx = [];
    for (let r = 0; r < RINGS; r++) {
        for (let m = 0; m < M; m++) {
    ...
    for (let m = 0; m < M; m++)
        idx.push(cn, (m + 1) % M, m); // faces -z
    const ct = capCentroid(RINGS - 1, 0.03);
    const base = (RINGS - 1) * M;
    for (let m = 0; m < M; m++)
        idx.push(ct, base + m, base + ((m + 1) % M)); // faces +z
 ...
 */
function buildPatch(grid, rows, cols, offset, yShift) {
    const pos = [];
    const idx = [];
 ...
 */
function sweep(path, shape, refFn, capStart = true, capEnd = true) {
    const n = path.length;
...
function ellipseShape(n, ru, rv, cv) {
    const pts = [];
    for (let i = 0; i < n; i++) {
...
// Main build
// ---------------------------------------------------------------------------
export function buildXC60Geometry() {
    const mats = {};
    for (const s of SLOT_NAMES)
        mats[s] = new THREE.MeshStandardMaterial({ name: s });
    const slots = {};
    const add = (parent, geo, slot, name) => {
    ...
    // Windshield: stations 7-9, full width, inset 0.01.
    {
        ...
        const cols = [];
        for (let j = J_P7; j <= J_P9; j++)
            cols.push({ j, s: 1 });
        for (let j = J_P9 - 1; j >= J_P7; j--)
            cols.push({ j, s: -1 });
    ...
    // ---- grille (concave panel proud of the fascia) + iron mark -----------
    {
        ...
        if (frame)
            add(body, frame, "trim", "grilleFrame");
    ...
    // ---- taillights (vertical L-blades, hull-hugging sweep) ---------------
    {
        ...
        for (const s of [1, -1]) {
            ...
            const ref = (p) => new THREE.Vector3(s * 0.9, 0.15, Math.max(0.12, (p.z - 1.55) * 0.9)).normalize();
